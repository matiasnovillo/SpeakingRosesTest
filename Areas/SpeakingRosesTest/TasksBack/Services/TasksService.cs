...
namespace SpeakingRosesTest.Areas.SpeakingRosesTest.TasksBack.Services
{
    public class TasksService : ITasksService
    {
        #region Exportations
        public void ExportToExcel(string path, DataTable dtTasks)
        {
            using var Book = new XLWorkbook();

            DataTable dtTasksOriginal = new()
            {
                TableName = "Tasks"
            };

            //We define another DataTable dtTasksCopy to avoid issue related to DateTime conversion
            DataTable dtTasksCopy = new()
            {
                TableName = "Tasks"
            };

            #region Define columns for dtTasksCopy
            DataColumn dtColumnTasksIdFordtTasksCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "TasksId"
            };
            dtTasksCopy.Columns.Add(dtColumnTasksIdFordtTasksCopy);

            DataColumn dtColumnActiveFordtTasksCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "Active"
            };
            dtTasksCopy.Columns.Add(dtColumnActiveFordtTasksCopy);

            DataColumn dtColumnDateTimeCreationFordtTasksCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "DateTimeCreation"
            };
            dtTasksCopy.Columns.Add(dtColumnDateTimeCreationFordtTasksCopy);

            DataColumn dtColumnDateTimeLastModificationFordtTasksCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "DateTimeLastModification"
            };
            dtTasksCopy.Columns.Add(dtColumnDateTimeLastModificationFordtTasksCopy);

            DataColumn dtColumnUserCreationIdFordtTasksCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "UserCreationId"
            };
            dtTasksCopy.Columns.Add(dtColumnUserCreationIdFordtTasksCopy);

            DataColumn dtColumnUserLastModificationIdFordtTasksCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "UserLastModificationId"
            };
            dtTasksCopy.Columns.Add(dtColumnUserLastModificationIdFordtTasksCopy);

            DataColumn dtColumnTitleFordtTasksCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "Title"
            };
            dtTasksCopy.Columns.Add(dtColumnTitleFordtTasksCopy);

            DataColumn dtColumnDescriptionFordtTasksCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "Description"
            };
            dtTasksCopy.Columns.Add(dtColumnDescriptionFordtTasksCopy);

            DataColumn dtColumnPriorityIdFordtTasksCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "PriorityId"
            };
            dtTasksCopy.Columns.Add(dtColumnPriorityIdFordtTasksCopy);

            DataColumn dtColumnDueDateFordtTasksCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "DueDate"
            };
            dtTasksCopy.Columns.Add(dtColumnDueDateFordtTasksCopy);

            DataColumn dtColumnStatusIdFordtTasksCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "StatusId"
            };
            dtTasksCopy.Columns.Add(dtColumnStatusIdFordtTasksCopy);

            
            #endregion

            dtTasksOriginal = dtTasks;

            foreach (DataRow DataRow in dtTasksOriginal.Rows)
            {
                dtTasksCopy.Rows.Add(DataRow.ItemArray);
            }

            var Sheet = Book.Worksheets.Add(dtTasksCopy);
        ...
        public void ExportToCSV(string path, List<Tasks> lstTasks)
        {
            using var Writer = new StreamWriter(path);

            using var CsvWriter = new CsvWriter(Writer, CultureInfo.InvariantCulture);

            CsvWriter.WriteRecords(lstTasks);
        }

        public void ExportToPDF(string path, List<Tasks> lstTasks)
        {
            ...
            {
                RowsAsHTML += $@"{Tasks?.ToStringOnlyValuesForHTML()}";
            }
    ...
    <tr>
        <th align=""left"" valign=""top"" style=""border-width: 1px; border-style: solid; border-color: #e8e8e8; border-top: none; border-left: none; border-right: none;"">
            <font face=""'Source Sans Pro', sans-serif"" color=""#000000"" style=""font-size: 20px; line-height: 28px; font-weight: 600;"">
                <span style=""font-family: 'Source Sans Pro', Arial, Tahoma, Geneva, sans-serif; color: #000000; font-size: 20px; line-height: 28px; font-weight: 600;"">TasksId&nbsp;&nbsp;&nbsp;</span>
            </font>
            ...
            <font face=""'Source Sans Pro', sans-serif"" color=""#000000"" style=""font-size: 20px; line-height: 28px; font-weight: 600;"">
                <span style=""font-family: 'Source Sans Pro', Arial, Tahoma, Geneva, sans-serif; color: #000000; font-size: 20px; line-height: 28px; font-weight: 600;"">TasksStatusId&nbsp;&nbsp;&nbsp;</span>
            </font>
            <div style=""height: 10px; line-height: 10px; font-size: 8px;"">&nbsp;</div>
        </th>
    </tr>
...
").SaveAs(path);
        ...
        public List<Tasks> ImportExcel(string path)
        {
            ...
            foreach (var row in Rows)
            {
                ...
                if (rowNumber > 1)
                {
                    string Title = row.Cell(7).GetString();
                    string Description = row.Cell(8).GetString();
                    int PriorityId = Convert.ToInt32(row.Cell(9).GetString());
                    DateTime DueDate = Convert.ToDateTime(row.Cell(10).GetString());
                    int StatusId = Convert.ToInt32(row.Cell(11).GetString());
