...
namespace SpeakingRosesTest.Areas.SpeakingRosesTest.StatusBack.Services
{
    public class StatusService : IStatusService
    {
        #region Exportations
        public void ExportToExcel(string path, DataTable dtStatus)
        {
            using var Book = new XLWorkbook();

            DataTable dtStatusOriginal = new()
            {
                TableName = "Status"
            };

            //We define another DataTable dtStatusCopy to avoid issue related to DateTime conversion
            DataTable dtStatusCopy = new()
            {
                TableName = "Status"
            };

            #region Define columns for dtStatusCopy
            DataColumn dtColumnStatusIdFordtStatusCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "StatusId"
            };
            dtStatusCopy.Columns.Add(dtColumnStatusIdFordtStatusCopy);

            DataColumn dtColumnActiveFordtStatusCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "Active"
            };
            dtStatusCopy.Columns.Add(dtColumnActiveFordtStatusCopy);

            DataColumn dtColumnDateTimeCreationFordtStatusCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "DateTimeCreation"
            };
            dtStatusCopy.Columns.Add(dtColumnDateTimeCreationFordtStatusCopy);

            DataColumn dtColumnDateTimeLastModificationFordtStatusCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "DateTimeLastModification"
            };
            dtStatusCopy.Columns.Add(dtColumnDateTimeLastModificationFordtStatusCopy);

            DataColumn dtColumnUserCreationIdFordtStatusCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "UserCreationId"
            };
            dtStatusCopy.Columns.Add(dtColumnUserCreationIdFordtStatusCopy);

            DataColumn dtColumnUserLastModificationIdFordtStatusCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "UserLastModificationId"
            };
            dtStatusCopy.Columns.Add(dtColumnUserLastModificationIdFordtStatusCopy);

            DataColumn dtColumnNameFordtStatusCopy = new()
            {
                DataType = typeof(string),
                ColumnName = "Name"
            };
            dtStatusCopy.Columns.Add(dtColumnNameFordtStatusCopy);

            
            #endregion

            dtStatusOriginal = dtStatus;

            foreach (DataRow DataRow in dtStatusOriginal.Rows)
            {
                dtStatusCopy.Rows.Add(DataRow.ItemArray);
            }

            var Sheet = Book.Worksheets.Add(dtStatusCopy);
        ...
        public void ExportToCSV(string path, List<Status> lstStatus)
        {
            using var Writer = new StreamWriter(path);

            using var CsvWriter = new CsvWriter(Writer, CultureInfo.InvariantCulture);

            CsvWriter.WriteRecords(lstStatus);
        }

        public void ExportToPDF(string path, List<Status> lstStatus)
        {
            ...
            {
                RowsAsHTML += $@"{Status?.ToStringOnlyValuesForHTML()}";
            }
    ...
    <tr>
        <th align=""left"" valign=""top"" style=""border-width: 1px; border-style: solid; border-color: #e8e8e8; border-top: none; border-left: none; border-right: none;"">
            <font face=""'Source Sans Pro', sans-serif"" color=""#000000"" style=""font-size: 20px; line-height: 28px; font-weight: 600;"">
                <span style=""font-family: 'Source Sans Pro', Arial, Tahoma, Geneva, sans-serif; color: #000000; font-size: 20px; line-height: 28px; font-weight: 600;"">StatusId&nbsp;&nbsp;&nbsp;</span>
            </font>
            ...
            <font face=""'Source Sans Pro', sans-serif"" color=""#000000"" style=""font-size: 20px; line-height: 28px; font-weight: 600;"">
                <span style=""font-family: 'Source Sans Pro', Arial, Tahoma, Geneva, sans-serif; color: #000000; font-size: 20px; line-height: 28px; font-weight: 600;"">StatusStatusId&nbsp;&nbsp;&nbsp;</span>
            </font>
            <div style=""height: 10px; line-height: 10px; font-size: 8px;"">&nbsp;</div>
        </th>
    </tr>
